...
//        ((NioSocketChannel)((MqttClientConnectionConfig)((MqttClientConfig)((Mqtt3ClientConfigView)((Mqtt3AsyncClientView)device1Client.client).clientConfig).delegate).connectionConfig.get()).channel).config().setOption(ChannelOption.SO_LINGER, 0I)
    ...
    def "Mqtt broker mTLS test"() {
        ...
        // The client retries, so it cycles through connecting states rather than settling on one;
        // what matters is that it never gets through
        def deadline = System.currentTimeMillis() + 3000
        while (System.currentTimeMillis() < deadline) {
            assert invalidClient.getConnectionStatus() != ConnectionStatus.CONNECTED
            Thread.sleep(100)
        }
